...
/** Lazily seeds panel credentials from config on first run, mirroring PanelAuth. */
...
export function savePanelAuth(store: SqliteStore, auth: PanelAuth): void {
  store.setJson(PANEL_AUTH_KEY, auth);
}
