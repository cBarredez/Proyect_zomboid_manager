...
import { verifyPassword, hashPassword } from "../security/passwordHasher.js";
import { createSessionProof, verifySessionProof } from "../security/sessionProof.js";
import { getOrSeedPanelAuth, savePanelAuth } from "../security/panelAuth.js";

export const SESSION_COOKIE = "pz_manager_session";
...
export async function registerAuthRoutes(app: FastifyInstance, ctx: AppContext): Promise<void> {
  ...
  app.put<{ Body: { currentPassword?: string; newUsername?: string; newPassword?: string } }>(
    ...
    async (req, reply) => {
      ...
      savePanelAuth(ctx.store, updated);
